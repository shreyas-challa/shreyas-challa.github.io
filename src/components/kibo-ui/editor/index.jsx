...
import { ReactRenderer, EditorProvider as TiptapEditorProvider, useCurrentEditor } from "@tiptap/react";
...
export const defaultSlashSuggestions =
  () => [
    {
      title: "Text",
    ...
    },
    {
      ...
      searchTerms: ["upload", "local", "file"],
      icon: BoldIcon,
      command: ({ editor, range }) => {
        // keep caret position but just trigger file picker
        if (typeof window !== 'undefined' && window.__openInlineImagePicker) {
          window.__openInlineImagePicker();
        } else {
          alert('Inline image picker not available here. Use the Add Inline Image button.');
        }
      },
    },
    {
...
const EditorSlashMenu = ({
  ...
  <Command
    ...
    <CommandEmpty
      className="flex w-full items-center justify-center p-4 text-muted-foreground text-sm">
      <p>No results</p>
    </CommandEmpty>
    <CommandList>
      {items.map((item) => (
        <CommandItem
          ...
          <div
            className="flex size-9 shrink-0 items-center justify-center rounded border bg-secondary">
...
export const EditorProvider = ({
  ...
  const defaultExtensions = [
    ...
    Image.configure({
      HTMLAttributes: {
        class: cn("rounded-md max-w-full h-auto my-4"),
      },
    }),
    Superscript,
    Subscript,
...
export const EditorLinkSelector = ({
  ...
  const defaultValue = (editor.getAttributes("link")).href;
...
export const EditorTableRowMenu = ({
  children
}) => {
  const { editor } = useCurrentEditor();
  const [top, setTop] = useState(0);
